...
function cancelReturnRequest(value)
{
	window.location="/Booksmania/welcome/cancelReturnRequest?requestId=".concat(value);
}
</script>
		...
		<div id="book-image"><img src="<c:url value="/resources/images/${userRequest.getBook().imageName} "/>" alt="${userRequest.getBook().imageName}" class="img-thumbnail"/></div>
			<div style="" id="request-details">
				<table id="requestDescription" class="table table-hover">
	        	    ...
	        	    <tr><td>
	        	    	<button type="button" class="btn btn-default" data-dismiss="modal" onclick="cancelReturnRequest(${userRequest.requestId})">Cancel Request! I want to Keep this book</button>
					</td></tr>
	              </table>
	        </div>
	 </div>
