...
<%@taglib uri="/WEB-INF/customFunctions.tld" prefix="function"%>

<script type="text/javascript">
function redirectReturnRequest(value)
{
	window.location="/Booksmania/welcome/returnRequest?requestId=".concat(value);
}
</script>
		...
		<div id="book-image"><img src="<c:url value="/resources/images/${userRequest.getBook().imageName} "/>" alt="${userRequest.getBook().imageName}" class="img-thumbnail"/></div>
			<div style="" id="request-details">
				<table id="requestDescription" class="table table-hover">
	        	    ...
	        	    <tr><td><button type="button" class="btn btn-default" data-dismiss="modal" onclick="redirectReturnRequest(${userRequest.requestId})">Return this Book</button></td></tr> 
	              </table>
	        </div>
	 </div>
</c:forEach>
